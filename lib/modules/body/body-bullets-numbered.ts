...
export const bodyBulletsNumbered: Module = {
  ...
  render: (p) => {
    ...
    // Unicode "negative circled digit" glyphs — same trick Schneider uses
    // (sch_email.html line 824, 835, 846): ❶❷❸❹❺❻❼❽❾❿ are part of the
    // Dingbat block, supported by every email-client font fallback (Arial,
    // Helvetica, Segoe UI, system-ui). Outlook renders them as proper
    // round circles because they're a font glyph, not a CSS shape — no
    // border-radius involved. Colour the glyph via `color:` and the ring
    // takes the colour, the digit stays white.
    ...
    const rows = steps
        ...
        (s, i) => `
          <tr>
            <td valign="top" width="28" align="center"
              style="width:28px;vertical-align:top;padding:0 0 14px 0;font-family:${FONT_STACK};font-size:22px;line-height:1.1;color:${bulletBg};mso-line-height-rule:exactly;">
              ${UNICODE_BULLETS[i] ?? String(i + 1)}
            </td>
            <td valign="top" width="12" style="width:12px;font-size:0;line-height:0;mso-line-height-rule:exactly;">&nbsp;</td>
            <td valign="top"
              style="font-family:${FONT_STACK};font-size:${TYPE_SCALE.body}px;line-height:1.55;color:${escapeAttr(p.textColor)};padding:1px 0 14px 0;vertical-align:top;">
              ${safeHtml(s)}
            </td>
          </tr>
        `
      ...
      .join("");
    // bulletFg (the white "inside" the circle) is preserved in schema but
    // not used for the glyph — the Unicode character already has the
    // negative space rendered as part of the dingbat itself.
    void bulletFg;
