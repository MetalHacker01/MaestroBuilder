...
export const bodyThreeColumns: Module = {
  ...
  schema: {
    ...
    col1Icon: { type: "image-url", label: "Icon 1 URL", default: "https://placehold.co/80x80/5B5BD6/ffffff/png?text=1", group: "Column 1" },
  ...
  render: (p) => {
    ...
    const renderCol = (icon: unknown, title: unknown, body: unknown, linkText: unknown, linkUrl: unknown) => `
      <mj-column width="33.33%" vertical-align="top" padding="0 8px">
        <mj-image src="${escapeAttr(icon)}" alt="" width="${Number(p.iconSize)}px" align="center" padding="0 0 12px 0" />
        <mj-text align="center" font-family="${FONT_STACK}" font-size="${TYPE_SCALE.h3}px" line-height="1.25" font-weight="700" color="${escapeAttr(p.titleColor)}" padding="0 0 6px 0">${safeHtml(title)}</mj-text>
        <mj-text align="center" font-family="${FONT_STACK}" font-size="${TYPE_SCALE.body}px" line-height="1.55" color="${escapeAttr(p.bodyColor)}" padding="0 0 8px 0">${safeHtml(body)}</mj-text>
    ...
    return `
      ${showHeadline ? `
        <mj-section background-color="${escapeAttr(p.bgColor)}" padding="${(p.padding as { t: number }).t}px ${(p.padding as { r: number }).r}px 12px ${(p.padding as { l: number }).l}px">
          <mj-column>
      ...
      ` : ``}
      <mj-section background-color="${escapeAttr(p.bgColor)}" padding="${showHeadline ? `0 ${(p.padding as { r: number }).r}px ${(p.padding as { b: number }).b}px ${(p.padding as { l: number }).l}px` : spacing(p.padding as never)}">
