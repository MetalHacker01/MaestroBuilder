<p align="center">
  <img src="./public/maestro-logo.png" width="130" alt="Maestro Builder" />
</p>
...
It is the modern replacement for an internal jQuery-era tool that produced 75 hand-tuned email modules across four `.json` files. Maestro Builder collapses that into ~30 typed-and-editable modules with a real preview, real send-test, and real dark-mode support.

## Highlights

- **30 production-ready modules**, including preheaders, logos, hero with background-image overlay (VML-bulletproof for Outlook), three-column feature rows, numbered steps, product cards with star ratings, promo banners with redemption codes, app-store badges, footers with configurable social slots, dividers and accent rules
...
- **Bulletproof Outlook** via `<v:roundrect>` rounded buttons, `<v:image>` + `<v:rect>` overlay-on-photo hero pattern, and `mso-padding-alt` throughout. All patterns are derived from documented Litmus and Email-on-Acid references
- **Dark-mode support**. Opt-in toggle that emits per-instance HSL-derived dark variants (red background becomes dark red, dark navy headline becomes soft light blue), plus `[data-ogsc]` / `[data-ogsb]` mirrors for Outlook.com and the meta tags Apple Mail requires
- **Send a test in two clicks** with Mailjet integration, including friendly error guidance for the validation-hold and sender-not-verified states. Falls back to a one-click "open in htmltest.email" with the HTML auto-copied to clipboard
- **No backend persistence**. `.html` downloads ship the email, `.json` downloads round-trip the editable state, and base64 URL state lets you share a draft over Slack
- **Bulletproof rounded CTAs** that survive Outlook 2007 using the canonical VML `<v:roundrect>` + `mso-hide:all` HTML fallback pattern, with arcsize calculated correctly from radius and height
...
Maestro sends test emails through Mailjet. The free tier gives you 200 sends per day and 6000 per month, well beyond what a typical QA loop needs.
...
**One-time Mailjet setup** (about 2 minutes):

1. Sign up at [app.mailjet.com](https://app.mailjet.com) (free tier, no card required)
2. **Senders & Domains**, then **Add a sender**, enter your email, then click the confirmation link Mailjet sends you
3. **Account Settings**, then **API Key Management**, copy the API Key and Secret Key
4. Paste them into `.env.local`, set `MAILJET_FROM_ADDRESS` to the email you verified in step 2, restart the dev server

If `MAILJET_FROM_ADDRESS` is unset, the API returns a 503 with guided setup steps that surface inside the Send-test dialog. A misconfigured environment never silently fails.
...
| Body (16) | headline-paragraph, headline-paragraph-cta, two-col-text, two-col-image-text, three-columns, bullets, bullets-numbered, image-card, quote, cta-only, cta-pair, **promo-banner** (offer + code chip), **product-card**, **product-review** (with star rating + customer quote), **stat-row** (3-number social proof), **app-badges** (App Store + Google Play) |
...
User edits  ─►  Zustand store ([instances])
       ...
       POST /api/render
                 ...
                 ▼  buildMjml() concatenates each module.render(props),
                    then injects mb-uid-{uid} class markers on each
                    section for click-to-select and per-instance
                    dark-mode rules
                 │
                 ▼  wrapMjml() adds the MJML shell:
                    1. meta color-scheme tags (Apple Mail dark-mode opt-in)
                    2. per-instance darkBg / lightText rules under
...
| `lib/modules/registry.ts` | Imports and exports all 30 modules + `getDefaultProps()` helper |
| `lib/modules/helpers.ts` | `bulletproofButton()`, `bulletproofBgImage()`, `bulletproofCard()` helpers + the `BRAND` design tokens |
| `lib/render/compile.ts` | `buildMjml(instances)`. Annotation and dispatch to module renders |
...
That is all. The palette, property panel, drag-drop, save/load, send-test, and dark-mode CSS pick it up automatically.

## Design system

| Token | Value |
|---|---|
| Accent | `#1d4ed8` (Tailwind blue-700, matches the landing page CTA) |
...
The canonical bulletproof background-image pattern uses `<v:image>` + `<v:rect position:absolute>` for the background, and nesting a `<v:roundrect>` for the overlay inside that breaks the layout in Outlook (the overlay button drifts to the document origin). Buttons, banners, and standalone CTAs still get rounded corners on Outlook via VML; only the hero overlay's container is squared on Outlook desktop. The hero CTA inside the overlay also uses a flat-table button (no v:roundrect) for the same reason.
...
Gmail iOS and Outlook 365 Windows desktop apply their own color inversion regardless of `prefers-color-scheme`. Maestro emits the correct media-query CSS and `[data-ogsc]` mirrors for clients that respect them, but you cannot override force-invert. Design the light version to read in those clients.
...
If the dev server hits the Turbopack `Cannot find module '[turbopack]_runtime.js'` error, it is the `.next` cache colliding with file syncing (OneDrive / iCloud / Dropbox). Fix:
...
MIT License with an attribution requirement and an open-source-only redistribution clause. See [`LICENSE`](./LICENSE) for the full text.

In short:

- You may use, copy, modify, fork, and redistribute Maestro Builder freely.
- Any public distribution (including forks, hosted copies, and enhanced versions) must credit **Aldorino Rrushi** in the README, in the application UI, and in the repository description on GitHub or similar platforms.
- Any public distribution must remain open source under an OSI-approved license. Closed-source redistribution is not permitted.

A suitable attribution reads:

> Based on Maestro Builder by Aldorino Rrushi (https://github.com/MetalHacker01/MaestroBuilder)

Questions about a use case that does not fit these terms: reach out via [LinkedIn](https://www.linkedin.com/in/aldorino-rrushi/) or arrushi@marketone.com.
